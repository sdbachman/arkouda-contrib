...
def array3D(val, m, n, p) -> Union[pdarray, Strings]:
    """
    ...
    if isinstance(val, bool):
        args = f"bool {val} {m} {n} {p}"
    elif isinstance(val, int):
        args = f"int64 {val} {m} {n} {p}"
    elif isinstance(val, float):
        args = f"float64 {val} {m} {n} {p}"

    rep_msg = generic_msg(cmd='array3d', args=args)
...
def randint3D(low : numeric_scalars, high : numeric_scalars, 
    ...
    repMsg = generic_msg(cmd='randint3d', args='{} {} {} {} {} {} {}'.\
                         format(dtype.name, lowstr, highstr, m, n, p, seed))
    return create_pdarray3D(repMsg)
